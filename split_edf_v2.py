...
def split_edf(input_file, out_dir_path, splitting_parts_num):
    ...
    def get_shortest_time_cut(signal_mass, fs_massive, cut_num):
        print('fs_mas = ' + str(fs_massive))
        timecut_mass = []
        for sig, fs in zip(signal_mass,fs_massive):
            print(fs)
            siglen = (len(sig)/fs).__floor__()
            total_timecut = (siglen/cut_num).__floor__()
    ...
    for parts in range(splitting_parts_num):
        ...
        outfpath = out_dir_path+file_name+'_'+ str(parts+1)+'.edf'
...
for file in files_list:
    ...
    split_edf(input_file_path, out_file_dir_path, split_parts)
